---
title: 论文地图
description: Agent Harness 相关研究的学术地图：按主题分组的代表论文、范式迁移时间轴，以及面向三类读者的阅读顺序建议。
---
...
::: tip 怎么用这张地图
如果你只有两小时，直接跳到[「三类读者的阅读顺序」](#三类读者的阅读顺序)。每篇论文都标了 arXiv 编号，年份与作者机构均已在发表页核实。同一主题下按时间排序，方便看出思路的演化。
:::
...
::: warning 一个共同的阅读纪律
这些论文里的绝对数字（成功率、pass@1）几乎全部依赖当时的模型版本与 harness 配置，横向比较意义有限。值得带走的是**机制与失败分析**，不是分数。
:::
...
- [核心论文导读](/papers/core-papers) — 少数几篇的逐篇精读
- [前沿动态](/papers/frontier) — 2024 年之后的新工作
